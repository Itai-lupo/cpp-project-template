...
def verfyFiles(files):
    ...
    for filename in (
        list(glob.iglob("src/**/*.c*", recursive=True))
        + list(glob.iglob("tests/**/**.c*", recursive=True))
        + list(glob.iglob("vendor/**/**.c*", recursive=True))
    ):
        if filename not in [file[0] for file in files]:
